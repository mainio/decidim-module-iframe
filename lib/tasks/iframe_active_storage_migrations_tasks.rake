  ...
  namespace :active_storage_migrations do
    ...
    task migrate_from_carrierwave: :environment do
      ...
      Decidim::Organization.find_each do |organization|
        ...
        routes_mappings = []
        Decidim::Iframe::MigrateLegacyImagesJob.perform_now(organization.id, routes_mappings)

        path = Rails.root.join("tmp/iframe_editor_images_mappings.csv")
    ...
    task check_migration_from_carrierwave: :environment do
      ...
      Decidim::CarrierWaveMigratorService.check_migration(
        klass: Decidim::Iframe::EditorImage,
        cw_attribute: "image",
        cw_uploader: Decidim::Cw::Iframe::ImageUploader,
        as_attribute: "file",
        logger: logger
      )
    end
