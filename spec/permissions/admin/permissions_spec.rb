...
module Decidim::Iframe::Admin
  describe Permissions do
    ...
    context "when accessing iframe config variables" do
      context "and config is enabled" do
        it { is_expected.to be true }
      end

      Decidim::Iframe.config.keys.each do |key|
        context "and config [#{key}] is disabled" do
          let(:feature) { key }

          before do
            allow(Decidim::Iframe.config).to receive(key).and_return(:disabled)
          end
    ...
    context "when accessing admin_accountability" do
      ...
      before do
        allow(Decidim::Iframe.config).to receive(feature).and_return(status)
      end
